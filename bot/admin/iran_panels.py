...
def show_create_token_result(call, token: str, expires_at: str, api_base_url: str) -> None:
    ...
    text = (
        ...
        "─────────────────────\n"
        "📋 <b>دستورالعمل نصب آفلاین روی سرور ایران:</b>\n\n"
        "1. فایل iran.zip را به سرور ایران منتقل کنید:\n"
        "<code>scp iran.zip root@IRAN_SERVER_IP:/root/</code>\n\n"
        "2. روی سرور ایران:\n"
        "<code>unzip iran.zip &amp;&amp; cd iran &amp;&amp; sudo bash install.sh</code>\n\n"
        "3. هنگام نصب این اطلاعات خواسته می‌شود:\n"
        f"   • API Base URL: <code>{esc(api_base_url)}</code>\n"
        f"   • Registration Token: <code>{esc(token)}</code>\n\n"
        "📌 <b>نکات مهم:</b>\n"
        "• سرور ایران <b>نیازی به اینترنت</b> برای نصب ندارد\n"
        "• فقط Python 3.8+ روی سرور کافی است\n"
        "• هیچ pip install یا apt install انجام نمی‌شود\n"
        "• اگر از حالت xray_vless استفاده می‌کنید،\n"
        "  مطمئن شوید فایل <code>iran/xray/xray</code>\n"
        "  (باینری لینوکس Xray) داخل بسته وجود دارد\n\n"
